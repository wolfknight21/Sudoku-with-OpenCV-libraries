...
def create_grid_mask(vertical, horizontal):
    # Combina las lineas verticales y horizontales del grid
    grid = cv2.add(horizontal, vertical)
    # Sobrepone y delimita el area completa
    ...
    # Busca las ubicaciones de las lineas, usando el array (rho, theta en radianes)
    pts = cv2.HoughLines(grid, .3, np.pi / 90, 200)

    lines = process_helpers.draw_lines(grid, pts)
    # Extrea la lineas de un solo numero
...
def find_contours(img, original):
    # Busca los contadores limite de la imagen
    ...
    # Busca si que es lo que se esta buscando en la matriz
    for cnt in contours:
    ...
    if polygon is not None:
        # Encuentra las esquinas de los bordes extremos
        ...
        # Si no es un cuadrado no se comparará
        ...
        # Dibuja los circulos correspondientes para las esquinas
...
def warp_image(corners, original):
    # Se deforman los puntos
    corners = np.array(corners, dtype='float32')
    top_left, top_right, bot_right, bot_left = corners

    # Se busca la mejor anchura, por lo que el cuadro se deforma mediante la altura = largo
    width = int(max([
    ...
    # Crea un array que toma el superior izquierdo, superior derecho, inferior izquierdo, inferior derecho
...
def split_into_squares(warped_img):
    ...
    # Busca cada cuadro asumiendo que estan del mismo lado
    for j in range(9):
        for i in range(9):
            p1 = (i * width, j * width)  # Toma la esquina superior izquierda del cuadro
            p2 = ((i + 1) * width, (j + 1) * width)  # Toma la esquina inferior derecha
...
def draw_digits_on_warped(warped_img, solved_puzzle, squares_processed):
    ...
    # Busca los cuadros ubicados en la matriz
    index = 0
    for j in range(9):
        for i in range(9):
            if type(squares_processed[index]) == int:
                p1 = (i * width, j * width)  # Toma la esquina superior izquierda del cuadro
                p2 = ((i + 1) * width, (j + 1) * width)  # Toma la esquina inferior del cuadro
...
#Deforma la imagen cuando ya se hizo el proceso de analisis en la red neuronal devolviendo la cadena de los datos a la imagen extraída de la cámara usando la librería findHomography de OPENCV
